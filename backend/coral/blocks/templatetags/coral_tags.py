...
from common.wagtailsettings_models import LayoutSettings
from ..base_blocks import CoralAdvSettings
from ..snippets import Footer, Navbar
# from settings import cr_settings, get_bootstrap_setting
...
def is_advanced_setting(obj):
    return CoralAdvSettings in (obj.__class__,) + obj.__class__.__bases__
...
def og_image(context, page):
    ...
    # Fixes #240 https://github.com/coderedcorp/issues/240
...
def process_form_cell(request, cell):
    if isinstance(cell, str) and cell.startswith('/protected/'):
...
@register.filter
def map_to_css_message(message_tag):
    """
    Converts a message level to a  alert class
    """
