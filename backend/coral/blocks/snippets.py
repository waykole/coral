...
from .models import HTML_STREAMBLOCKS, NAVIGATION_STREAMBLOCKS, CONTENT_STREAMBLOCKS
...
class Category(ClusterableModel):
    ...
    panels = [
        FieldPanel('name'),
        InlinePanel('terms', label=_('Category Terms'))
    ]
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            suffix = 1
            while True:
                if not Category.objects.filter(slug=tmpslug).exists():
                    self.slug = tmpslug
                    break
...
class CategoryTerm(Orderable, models.Model):
    """
    Term used to categorize a page.
    """
    class Meta:
        verbose_name = _('Category Term')
        verbose_name_plural = _('Category Terms')
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            suffix = 1
            while True:
                if not CategoryTerm.objects.filter(slug=tmpslug).exists():
                    self.slug = tmpslug
                    break
                tmpslug = newslug + "-" + str(suffix)
                suffix += 1
...
class CoralEmail(ClusterableModel):
    """
...
@register_snippet
class CoralAdvert(models.Model):
