...
from .base_blocks import (
    ...
    BaseLinkBlock,
    ButtonMixin,
    CategoryTermChooserBlock,
    CoralAdvTrackingSettings,
    LinkStructValue,
)
...
class PageListBlock(BaseBlock):
    ...
    categorized_by = CategoryTermChooserBlock(
        required=False,
        label=_('Categorized as'),
        help_text=_('Only show pages with this category.')
    )
    ...
    def get_context(self, value, parent_context=None):
        ...
        # try to use the CoralPage `get_index_children()`,
        # but fall back to get_children if this is a non-CoralPage
        if hasattr(indexer, 'get_index_children'):
            pages = indexer.get_index_children()
            if value['categorized_by']:
                try:
                    pages = pages.filter(category_terms=value['categorized_by'])
                except AttributeError:
                    # `pages` is not a queryset, or is not a queryset of CoralPage.
                    logger.warning(
                        "Tried to filter by Category in PageListBlock, but <%s.%s ('%s')>.get_index_children()  # noqadid not return a queryset or is not a queryset of CoralPage models.",  # noqa
